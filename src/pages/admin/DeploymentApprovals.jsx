    ...
    <div className="space-y-6">
      <section className="overflow-hidden rounded-3xl border border-slate-800 bg-gradient-to-br from-slate-950 via-slate-900 to-emerald-950 px-6 py-6 text-white shadow-soft sm:px-8">
        <div className="max-w-3xl">
          <p className="text-xs uppercase tracking-[0.22em] text-emerald-200">Deployment manager workflow</p>
          <h1 className="mt-3 text-3xl font-semibold tracking-tight sm:text-4xl">
            Deployment approvals and notifications
          </h1>
          <p className="mt-4 max-w-2xl text-sm leading-6 text-slate-300 sm:text-base">
            Review talent acquisition requests, approve or reject them, and materialize active deployments when they are ready.
          </p>
        </div>
      </section>
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        {[
          { label: "All requests", value: stats.total, note: "Talent acquisition submitted volume." },
                  ...
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-xs text-slate-500">
                        Requested by {request.requester || "Talent Acquisition"} - {request.targetType}: {request.targetName || "Unassigned"}
                      </p>
                    </div>
