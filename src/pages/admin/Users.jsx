...
export default function AdminUsers() {
  ...
  const stats = useMemo(() => {
    ...
    return {
      ...
      deploymentManagers: activeUsers.filter((user) => user.role === "DeploymentManager").length,
    ...
    <div className="space-y-6">
      <section className="overflow-hidden rounded-3xl border border-slate-800 bg-gradient-to-br from-slate-950 via-slate-900 to-cyan-950 px-6 py-6 text-white shadow-soft sm:px-8">
        <div className="max-w-3xl">
          <p className="text-xs uppercase tracking-[0.22em] text-cyan-300">Administrator Console</p>
      ...
      <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-5">
        ...
        <MetricCard label="Deployment Managers" value={formatNumber(stats.deploymentManagers)} tone="amber" />
        <MetricCard label="Talent Acquisition" value={formatNumber(stats.recruiters)} tone="amber" />
