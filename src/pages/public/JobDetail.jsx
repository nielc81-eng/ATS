...
export default function PublicJobDetail() {
  ...
  const handleApply = () => {
    ...
    const profileState = readCandidateProfileState(
      session?.email || "",
      session,
      resumeProfile
    );
    const result = applyToJob(job.id, {
      ...
      name: session?.name,
      resumeProfile,
      profileSkills: profileState?.profile?.skills || [],
    });
      ...
      <section className="surface-card p-6 sm:p-8">
        ...
        {isRecruiter ? (
          <p className="mt-4 text-sm text-amber-700">
            Talent Acquisition accounts can browse this board but cannot submit candidate applications.
          </p>
        ) : null}
