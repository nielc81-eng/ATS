...
} from "../lib/recruitmentCategoryApplicants";
...
function normalizeTimelineEntry(rawEntry = {}) {
  return {
    ...
    toStatus: isApplicationStatus(rawEntry.toStatus)
      ? rawEntry.toStatus
...
function normalizeApplication(rawApplication = {}, fallbackEmail = "") {
  ...
  if (!status) {
    status = rawTimeline.at(-1)?.toStatus || APPLICATION_STATUS.Submitted;
  }
  ...
  const initialRole =
    rawApplication.status === "Withdrawn" || status === APPLICATION_STATUS.Submitted
      ? "Candidate"
      : "System";
...
function isShortlistedStatus(status) {
  return PIPELINE_SHORTLIST_STATUSES.has(status);
}
...
export function RecruitmentDataProvider({ children }) {
  ...
  const updateJob = useCallback((jobId, payload = {}) => {
    ...
    if (!normalizedJobId) {
      return { ok: false, code: "MISSING_JOB_ID", message: "Job id is required." };
    }

    const existingJob = jobs.find((job) => job.id === normalizedJobId);
    if (!existingJob) {
    ...
    const nextJob = normalizeJob({
      ...
      analytics: existingJob.analytics,
    });

    setJobs((prev) => prev.map((job) => (job.id === normalizedJobId ? nextJob : job)));

    setApplicationsByEmail((prev) => {
      const nextEntries = Object.entries(prev).map(([email, applications]) => [
        email,
        ensureArray(applications).map((application) =>
          application.jobId === normalizedJobId
            ? { ...application, jobTitle: nextJob.title }
            : application
        ),
      ]);
  ...
  const deleteJob = useCallback((jobId) => {
    ...
    const targetJob = jobs.find((job) => job.id === normalizedJobId);
    if (!targetJob) {
      return { ok: false, code: "JOB_NOT_FOUND", message: "Job not found." };
    }

    const relatedApplications = Object.values(applicationsByEmail)
      .flat()
      .filter((application) => application.jobId === normalizedJobId);
  ...
  const getCategoryApplicantCounts = useCallback(
    () => deriveCategoryApplicantCounts(jobs, allApplications),
    [allApplications, jobs]
  );

  const getApplicantsByCategory = useCallback(
    (category) => deriveApplicantsByCategory(jobs, allApplications, category),
    [allApplications, jobs]
  );
  ...
  const applyToJob = useCallback(
    (jobId, payload = {}) => {
      ...
      const candidateSkills = [
        ...ensureArray(payload.resumeProfile?.skills),
        ...ensureArray(payload.profileSkills),
      ];
      const { matchedToRequirements, initialStatus } = getInitialApplicationStatus(
        candidateSkills,
        targetJob.mustHaveSkills
      );
      const timelineNote = matchedToRequirements
        ? "Matching in job posting requirements. Candidate moved to initial interview phase."
        : "Matching in job posting requirements not met. Candidate pooled for future opportunities.";
      ...
      const application = {
        ...
        updatedOn: appliedOn,
        timeline: [
          createSubmittedTimelineEntry(appliedOn),
          {
            at: appliedOn,
            byRole: "System",
    ...
    (applicationId, nextStatus, note = "", options = {}) => {
      ...
      if (
        (normalizedStatus === APPLICATION_STATUS.Rejected ||
          normalizedStatus === APPLICATION_STATUS.BackoutArchived) &&
        !trimmedNote
      ) {
